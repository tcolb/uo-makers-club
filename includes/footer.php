  <!-- sticky footer credit
       https://css-tricks.com/couple-takes-sticky-footer/ -->
<div class="push"></div>
<footer class="footer">

  <div class="container-fluid">
    <!-- spacing to get rightmost fb logo img -->
    <div class="row">
      <div class="col-1"></div>
      <div class="col-7">
        <span class="text-muted">University of Oregon 2017</span>
      </div>
      <div class="col-2">
        <a target="_blank" href="https://makerclubuo.slack.com/">
          <!-- logo from https://simpleicons.org/ -->
          <img src="static/images/slack.svg" class="" style="max-height:30px; min-height:30px; opacity:0.6"/>
        </a>
      </div>
      <div class="col-2">
        <a target="_blank" href="https://www.facebook.com/groups/871519893028368/">
          <!-- logo from https://simpleicons.org/ -->
          <img src="static/images/facebook.svg" class="" style="max-height:30px; min-height:30px; opacity:0.6"/>
        </a>
      </div>
    </div>
  </div>

</footer>

</body>
</html>
